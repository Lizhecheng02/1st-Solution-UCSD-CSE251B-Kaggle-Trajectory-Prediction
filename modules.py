...
class TrajectoryLSTMAttention(nn.Module):
    def __init__(
        ...
        self.social_attention = nn.MultiheadAttention(embed_dim=d_model, num_heads=nhead, dropout=dropout, batch_first=False)
    ...
    def forward(self, ego_input, all_agents_input, valid_agents_mask=None):
        ...
        all_agent_embeddings = agent_embeddings.view(batch_size * num_agents, seq_len, -1)
        _, (h_n, _) = self.lstm_encoder(all_agent_embeddings)
        h_n = h_n[-1].view(batch_size, num_agents, -1)

        ego_query = h_n[:, 0:1, :]
        query = ego_query.transpose(0, 1)
        key = value = h_n.transpose(0, 1)
        attn_output, _ = self.social_attention(query, key, value)
        context = attn_output.transpose(0, 1).squeeze(1)

        h_0 = context.unsqueeze(0).repeat(self.trajectory_decoder.num_layers, 1, 1)
